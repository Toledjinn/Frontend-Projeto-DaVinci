...
export default function CollapsiblePeriogramItem({ item, isOpen, onToggle }: CollapsiblePeriogramItemProps) {
  return (
    <View style={styles.container}>
      ...
      {isOpen && (
        <View style={styles.content}>
          ...
          {upperArchTeeth.map(tooth => <ToothDetail key={tooth} toothNumber={tooth} data={item.data[tooth] || {}} />)}
          
          <Text style={styles.sectionTitle}>Arco Inferior</Text>
          {lowerArchTeeth.map(tooth => <ToothDetail key={tooth} toothNumber={tooth} data={item.data[tooth] || {}} />)}
        </View>
      )}
    </View>
  );
