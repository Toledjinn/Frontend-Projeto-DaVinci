...
export type PageName = 'chefinho' | 'escova' | 'pasta' | 'fioDental' | 'fluor' | 'revelador';
...
const educationalContentSlice = createSlice({
  ...
  initialState,
  reducers: {
    // Reducer para atualizar a página inteira
    updatePage: (state, action: PayloadAction<{ page: PageName; newSlides: CarouselSlide[] }>) => {
      state.pages[action.payload.page] = action.payload.newSlides;
    },
    // Reducer para adicionar um slide
    addSlide: (state, action: PayloadAction<{ page: PageName; layoutKey: string }>) => {
      const { page, layoutKey } = action.payload;
      ...
      state.pages[page].push(newSlide);
    },
    // Reducer para remover um slide
    removeSlide: (state, action: PayloadAction<{ page: PageName; slideId: string }>) => {
      ...
      if (pageToUpdate.length > 1) {
        state.pages[page] = pageToUpdate.filter((slide) => slide.id !== slideId);
      } else {
        // Alerta não pode ser feito no reducer, mas a lógica de impedir a remoção do último slide está aqui.
        // O alerta precisaria ser gerenciado na camada da UI.
        console.warn('Não é possível remover o último slide.');
      }
    },
    // Adicionei um reducer para atualizar um slide individualmente, já que é uma funcionalidade comum e pode ser útil.
    updateSlide: (state, action: PayloadAction<{ page: PageName; slideId: string; newContent: Partial<CarouselSlide> }>) => {
      ...
      const slides = state.pages[page];
      const slideIndex = slides.findIndex(slide => slide.id === slideId);
      if (slideIndex !== -1) {
        slides[slideIndex] = { ...slides[slideIndex], ...newContent };
      }
    }
  },
});

export const { updatePage, addSlide, removeSlide, updateSlide } = educationalContentSlice.actions;
