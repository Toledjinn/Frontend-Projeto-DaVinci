...
import { loadAllSocialPages, saveAllSocialPages, saveSocialPage } from '@/data/socialDb';
...
const DEFAULT_PAGES: Record<PageName, SocialContent[]> = {
  oQueE: [
    {
      id: 'oqe_1',
      type: 'text',
...
export const useSocialStore = create<SocialState>((set, get) => ({
  ...
  hydrate: async () => {
    ...
    const loaded = await loadAllSocialPages();
    if (loaded) {
      set({ pages: loaded, isHydrated: true });
    } else {
      await saveAllSocialPages(DEFAULT_PAGES);
      set({ isHydrated: true });
    }
  },
  ...
  updatePage: (page, newContent) => {
    set((state) => {
      const nextPages = { ...state.pages, [page]: newContent };
      saveSocialPage(page, newContent);
  ...
  addBlock: (page, type) => {
    set((state) => {
      ...
      const next = [...state.pages[page], newBlock];
      saveSocialPage(page, next);
  ...
  removeBlock: (page, blockId) => {
    set((state) => {
      const next = state.pages[page].filter((block) => block.id !== blockId);
      saveSocialPage(page, next);
